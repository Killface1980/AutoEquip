...
namespace AutoEquip
{
    public class PawnCalcForApparel
    {
        ...
        private void OptimizeFromList(ref bool changed)
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < _allApparelsItems.Count; i++)
                {
                    ...
                    if (!apparel.IsForbidden(_saveablePawn.Pawn) && _outfit.filter.Allows(apparel)) // added only storage items EXPERIMENTAL
                    {
        ...
        private float OLD_CalculateStatsForCompare(Apparel ignore)
        {
            ...
            foreach (Saveable_Pawn_WorkStatDef workstat in _workstats)
            {
